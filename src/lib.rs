...
// Define the DuoInt struct for arbitrary-precision base-12 integer arithmetic
#[derive(Clone, Debug, PartialEq, Eq)]
struct DuoInt {
    digits: Vec<u8>,  // little-endian, 0-11, no leading zeros except for 0
    negative: bool,
}
...
impl DuoInt {
    ...
    fn multiply_by_power(&self, power: usize) -> Self {
        ...
        // Normalize: remove leading zeros
        while res.digits.len() > 1 && *res.digits.last().unwrap() == 0 {
            res.digits.pop();
        }
        res
    }
...
fn add_abs_digits(a: &Vec<u8>, b: &Vec<u8>) -> Vec<u8> {
    ...
    for i in 0..max_len {
        let sum = carry + if i < a.len() { a[i] as u32 } else {0} + if i < b.len() { b[i] as u32 } else {0};
...
fn div_abs_digits(dividend: &Vec<u8>, divisor: &Vec<u8>) -> Vec<u8> {
    ...
    let mut q = DuoInt::zero();
    let one = DuoInt::one();
    let mut rem = DuoInt { digits: dividend.clone(), negative: false };
    let div = DuoInt { digits: divisor.clone(), negative: false };
...
// Define DuoFixed for fixed-point base-12 arithmetic
#[derive(Clone, Debug, PartialEq)]
struct DuoFixed {
    value: DuoInt,  // Scaled value (integer * 12^scale)
    scale: usize,  // Number of fractional digits
}
...
impl DuoFixed {
    ...
    pub fn from_f64(f: f64, scale: usize) -> Self {
        let scaled = (f * 12f64.powi(scale as i32)).round() as i64;
        let negative = scaled < 0;
        let abs_scaled = scaled.abs().to_string();
        let mut digits = Vec::new();
        for c in abs_scaled.chars().rev() {
            digits.push(c as u8 - b'0');
        }
        if digits.is_empty() {
            digits.push(0);
        }
        let value = DuoInt { digits, negative };
        Self { value, scale }
    }
    ...
    // Implement sub, div similarly if needed
    // For harmonic reset: example function
    pub fn harmonic_reset(&mut self, cycle: usize) -> Self {
        if cycle % 12 == 0 {
            // Simple reset: normalize to avoid drift (placeholder; extend for AI weights)
...
// DecInt for base-10 arbitrary-precision, for comparison
#[derive(Clone, Debug, PartialEq, Eq)]
struct DecInt {
    digits: Vec<u8>,  // little-endian, 0-9, no leading zeros except for 0
    negative: bool,
}
...
// VM for DuoInt (with middleware, base-12)
#[derive(Clone)]
enum Instruction {
...
// DecVM for DecInt (without middleware, base-10)
#[derive(Clone)]
enum DecInstruction {
...
pub fn tune_weights(weights: Vec<f64>, scale: usize) -> Vec<f64> {
    ...
    for &w in &weights {
        ...
        df = df.harmonic_reset(12);  // Apply reset
...
pub fn evaluate_drift(weights: Vec<f64>, iterations: usize, scale: usize, with_tuning: bool) -> f64 {
    ...
    for i in 1..=iterations {
        ...
        if with_tuning && i % 12 == 0 {
            current = tune_weights(current, scale);
        }
    }
